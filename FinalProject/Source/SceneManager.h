...
private:
	...
	// can now accept transformation matrix as well
	void SetTransformations(
...
public:
	...
	// custom function for drawing the square frustum shapes of the ziggurat
	//void DrawZiggurat(glm::vec3 basePosition, float baseWidth, float baseDepth, int levels);

	glm::vec3 ConfigShape(glm::vec3 scaleXYZ, glm::vec3 positionXYZ, SHAPE typeShape, glm::vec3 rotationXYZ = glm::vec3(0.0f, 0.0f, 0.0f));

	void DrawShape(SHAPE typeShape);
	...
	void AddRamp(RAMP rampType, glm::vec3 scaleXYZ, glm::vec3 initialPositionXYZ, glm::vec3 rotationXYZ = glm::vec3(0.0f, 0.0f, 0.0f));
