...
namespace
{
	...
	static float gLastX = WINDOW_WIDTH / 2.0f;  // static / persistence added
	static float gLastY = WINDOW_HEIGHT / 2.0f;  // static / persistence added
	static bool gFirstMouse = true;  // static / persistence added

	// time between current frame and last frame
	float gDeltaTime = 0.0f; 
	float gLastFrame = 0.0f;

	// adjusts speed / camera sensitivity
	const float cameraSpeed = 1.0f; // mentioned in the processInput function of readings
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// this callback is used to receve mouse scrolly wheel events
...
void ViewManager::Mouse_Position_Callback(GLFWwindow* window, double xMousePos, double yMousePos)
{
	...
	const float camera_sensitivity = 0.1f; // modifier value to help with sensitivity
	yOffset *= camera_sensitivity;
	xOffset *= camera_sensitivity;



	if (g_pCamera) // while cam is valid
	{
 ...
 *
 *  This method is automatically called from GLFW whenever
 *  the mouse scroll wheel is scrolled within the active GLFW display window.
 *
 *	Based from Mouse_Position_Callback, added decl. in header
 *	Sourced from scroll input of glfw input guide
 ***********************************************************/
void ViewManager::Mouse_Scroll_Callback(GLFWwindow* window, double xMousePos, double yMousePos)
{
	// relies on the processMouseScroll method from Camera class defn. 
	if (g_pCamera)
	{
		g_pCamera->ProcessMouseScroll(static_cast<float>(yMousePos));
	}
